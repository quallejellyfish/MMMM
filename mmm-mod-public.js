...
let STATS_KEY = localStorage.getItem("statsKey") || "";
let statsUploadTimer = null;
let countedThisPlay = false;

async function fetchSongs() {
  try {
    const response = await fetch(`${API_BASE}/public/songs`);
...
async function fetchLyrics(songId) {
  ...
  try {
    const response = await fetch(`${API_BASE}/public/lyrics/${songId}`);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const data = await response.json();
    lyricsCache[songId] = data;
    const keys = Object.keys(lyricsCache);
    if (keys.length > 100) delete lyricsCache[keys[0]];
    return data;
  } catch (err) {
...
async function uploadStats() {
  ...
  if (!STATS_KEY) {
    console.warn("No stats key set.");
    return;
  }
  try {
    const response = await fetch(`${API_BASE}/public/stats/upload`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ key: STATS_KEY, stats }),
    });
    if (!response.ok) {
...
function updateStatsStatus() {
  ...
  if (statusEl) {
    statusEl.textContent = STATS_KEY ? `Key: ${STATS_KEY}` : "Key: not set";
  }
}
if (STATS_KEY) {
  document.getElementById("statsKeyInput").value = STATS_KEY;
}
updateStatsStatus();

document
  .getElementById("setStatsKeyBtn")
  .addEventListener("click", function () {
    const input = document.getElementById("statsKeyInput");
    const key = input.value.trim();
    if (key) {
      STATS_KEY = key;
      localStorage.setItem("statsKey", key);
      updateStatsStatus();
      showNotification(`Stats key set to "${key}"`, "system");
    } else {
      showNotification("Please enter a key", "system");
    }
  });

document
  .getElementById("uploadStatsBtn")
  .addEventListener("click", function () {
    if (!STATS_KEY) {
      showNotification("No stats key set", "system");
      return;
    }
    uploadStats()
